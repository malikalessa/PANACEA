...
class Ensemble_Configuration():
    ...
    def Model_Ensembling_Train_Medoids(self,x_train,x_test,y_train,y_test, model_medoids):
        ...
        #model_medoids = DalexDatasets.Clustering_Medoids(self)
        New_XTraining = []
        New_XValidation = []
        ...
        Save_Ensemble_Model.save_model(testPath,'ensembled_model_medoids.h5')
        return model



    def Model_Ensembling_load_models(self, name):
        # This Function is used to load the saved ensembled models in order to predict them
        # concatenating the new datasets (T+A)
        testPath =  self.ds.get('pathModels')
        # To load the Ensemble Model
        model = load_model(testPath+name)
        return model
    ...
    def Model_Ensembling_Train_Sequential(self,x_train,x_test,y_train,y_test):
        ...
        Save_Ensemble_Model.save_model(testPath, 'ensembled_model_sequential.h5')
        return model



    def create_ensemble_voting_medoids(self,x_train,x_test,y_train,y_test):
        model_medoids = DalexDatasets.Clustering_Medoids(self)
    ...
    def prediction(self,x_test,y_test, model, name):
