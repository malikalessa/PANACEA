...
def NN( params):
    ...
    class_weight = class_weight.compute_class_weight(class_weight="balanced",
                                                     classes=np.unique(YTraining),
                                                     y=YTraining)
    class_weight = dict(enumerate(class_weight))
    print('Class_Weight : ', class_weight)

    YTraining = np_utils.to_categorical(YTraining, len(np.unique(y_train)))
    YValidation = np_utils.to_categorical(YValidation, len(np.unique(y_train)))

    h = model.fit(XTraining, YTraining, batch_size=params["batch"], epochs=150, verbose=2, callbacks=callbacks_list
                  , shuffle= True , validation_data=(XValidation, YValidation))#, class_weight=class_weight)
...
def hypersearch( train_scaler, y_train, test_scaler, y_test,testPath,config_No):
    ...
    best = fmin(fit_and_score, space, algo=tpe.suggest, max_evals=20, trials=trials, rstate=np.random.RandomState(my_seed))
