...
def Individual_Models_Predictions():
    # Predicting individual Models
     testPath = gc.NSL_KDD_Models

     train_scaler, test_scaler, y_train, y_test = Preprocessing.read_NSL_KDD_Dataset()
...
def Model_Ensembling_Train_Adversarial_NSL_KDD_Medoids():
...
def create_models_NSL_KDD():
    testPath = gc.NSL_KDD_Models

    train_scaler, test_scaler, y_train, y_test = Preprocessing.read_NSL_KDD_Dataset()
    for i in range(gc.n_models):
        train_dataset_adv,y_train_adv = Preprocessing.read_T_A_Datasets_NSL(train_scaler,y_train,i)
